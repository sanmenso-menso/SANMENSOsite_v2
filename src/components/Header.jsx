...
const Header = ({ onNavigate, isOpening }) => {
    ...
    const numuIcon = '/images/numunumu_icon.webp';
    ...
    return (
        <motion.header
            ...
            style={!isOpening ? { y } : {}}
            className="fixed top-0 left-0 px-4 py-3 md:p-8 z-50 pointer-events-auto"
        >
            <button
                type="button"
                aria-label="ホームへ戻る"
                onClick={() => onNavigate('home')}
                className="group flex flex-row items-center gap-3 md:gap-4 text-left"
            >
                <img src={isNumunumuMode ? numuIcon : "/images/SANMENlogotoumei.webp"} alt={isNumunumuMode ? numuText : "SANMENSO logo"} className="w-auto h-10 md:h-11"/>
