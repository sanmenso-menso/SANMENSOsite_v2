...
const WorksPage = ({ initialFilter = 'all' }) => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 py-8 min-h-screen flex flex-col">
            <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-8 border-b-4 border-black pb-6 border-double bg-white/80 backdrop-blur-sm p-4">
                <div>
                    <h2 className="text-6xl md:text-8xl font-black tracking-tighter leading-none mix-blend-hard-light text-[#FFD700] drop-shadow-[4px_4px_0px_rgba(0,0,0,1)] font-sans" style={{ WebkitTextStroke: '2px black' }}>{isNumunumuMode ? numuText : 'WORKS'}</h2>
                    ...
                    {currentWorks.map((work) => (
                        <motion.div key={work.id} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ duration: 0.3 }} onClick={() => setSelectedWork(work)} className="group relative bg-white border-4 border-black p-4 h-full flex flex-col transition-all duration-300 hover:-translate-y-2 hover:rotate-1 hover:shadow-[12px_12px_0px_#000] shadow-[6px_6px_0px_#000] cursor-pointer overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-2 border-b-2 border-black pb-2 border-dashed">
                                <h3 className="text-2xl font-black tracking-tight leading-none font-sans">{work.title}</h3>
                            ...
                            <p className="text-sm font-medium leading-snug opacity-80 line-clamp-3 mb-4 flex-grow font-sans">{work.desc}</p>
                ...
                {selectedWork && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8">
                        ...
                        <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="relative w-full max-w-5xl h-[90vh] bg-[#f0f0f0] border-[6px] border-black shadow-[16px_16px_0px_#000] flex flex-col md:flex-row overflow-hidden z-10">
                            ...
                            <div className="w-full md:w-3/5 overflow-y-auto bg-white p-8 md:p-12 flex flex-col gap-8">
                                <div>
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        <span className="flex items-center gap-1 font-mono text-sm border border-black px-2 py-0.5 bg-yellow-300"><Tag size={12} /> {isNumunumuMode ? numuText : selectedWork.type.toUpperCase()}</span>
                                        <span className="flex items-center gap-1 font-mono text-sm border border-black px-2 py-0.5 bg-white"><Calendar size={12} /> {isNumunumuMode ? numuText : selectedWork.year}</span>
                                        <span className="flex items-center gap-1 font-mono text-sm border border-black px-2 py-0.5 bg-white"><User size={12} /> {isNumunumuMode ? numuText : selectedWork.role}</span>
                                    </div>
                                    <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-none mb-4 font-sans">{selectedWork.title}</h2>
